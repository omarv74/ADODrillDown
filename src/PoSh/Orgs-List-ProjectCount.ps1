...
$csvOutputPath = Join-Path -Path $scriptDir -ChildPath "OrgsProjectCount_$timestamp.csv"

# Open CSV file for writing
$csvOutputWriter = New-Object System.IO.StreamWriter($csvOutputPath)
$csvOutputWriter.WriteLine("OrganizationName,OrgOwnerEmail,ProjectCount")
...
foreach ($csvOrg in $csvOrgsList) {
    ...
    try {
        ...
        $csvOutputWriter.WriteLine("$orgName,$orgOwnerEmail,$($responseGroupsList.Count)")
